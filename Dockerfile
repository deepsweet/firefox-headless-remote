...
RUN apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends --yes install firefox=92\* dumb-init curl build-essential ruby ruby-dev gem socat wget fontconfig && \
    groupadd firefox && \
...
# Install firefox
RUN curl --location "https://ftp.mozilla.org/pub/firefox/releases/92.0b9/linux-x86_64/en-US/firefox-92.0b9.tar.bz2" \
